...
class User(UserMixin, client_db.Model):
    ...
    reviews = client_db.relationship("Review", backref=client_db.backref("user"))
...
class Review(client_db.Model):
    user_id = client_db.Column(client_db.Integer, client_db.ForeignKey("user.id"), primary_key=True)
    product_id = client_db.Column(client_db.Integer, client_db.ForeignKey("product.id"), primary_key=True)
